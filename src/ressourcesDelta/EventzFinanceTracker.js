...
import { projectTodoMutationRessources } from '../todoMutationRessources/todoMutationRessourcesProjection';
...
import handleUpdateEntry from './updateEntry/handleUpdateEntry';
import { v4 as uuidv4 } from 'uuid';
import './FinanceTracker.css';
import { incomeOptions, expenseOptions } from '../ressourceConfig';

const EventzFinanceTracker = ({ showChangeHistory = false, setShowChangeHistory = () => {} } = {}) => {
    const [updateForm, setUpdateForm] = useState({ entryId: '', newStartMonth: '', newEndMonth: '' });
  ...
  // Use workflow gating logic (legacy)
  // You may want to update this to your preferred workflow gating
  // For now, revert to previous gating logic
  ...
  const isStep4Ouverte = true;
  const canEditEntries = isStep4Ouverte;
  ...
  // State for periods query result
  ...
  function handlePeriodQuery() {
    if (periodResult) {
      setPeriodResult(null);
      return;
    }
    if (!latestDroitsPeriod) {
      setPeriodResult('No droits period');
      return;
    }
    // Compute the period for the latest changeId
    const latestChangeId = [...eventLog].reverse().find(e => e.changeId)?.changeId;
    if (!latestChangeId) {
      setPeriodResult('No changeId found');
      return;
    }
    const relevantEvents = eventLog.filter(e =>
      e.changeId === latestChangeId &&
      ['EntryAdded', 'EntryDeleted', 'EntryUpdated'].includes(e.event)
    );
    const months = [];
    relevantEvents.forEach(e => {
      if (e.event === 'EntryAdded' || e.event === 'EntryDeleted') {
        months.push(e.payload.startMonth);
      } else if (e.event === 'EntryUpdated') {
        months.push(e.payload.startMonth);
        // Add 1 month to endMonth
        const [year, month] = e.payload.endMonth.split('-').map(Number);
    ...
    if (months.length === 0) {
      setPeriodResult('No relevant events found');
      return;
    }
    const minMonth = months.sort()[0];
    setPeriodResult({ startMonth: minMonth, endMonth: latestDroitsPeriod.endMonth });
  }
  ...
  function handleUpdate(entryId) {
    setUpdateForm({ ...updateForm, entryId });
  }
  function handleUpdateSubmit(e) {
    ...
    try {
      ...
      const command = UpdateEntryCommand({
        ...
        changeId,
        newStartMonth: updateForm.newStartMonth,
        newEndMonth: updateForm.newEndMonth
      });
      const newEvents = handleUpdateEntry(eventLog, command, userEmail);
      const updated = [...eventLog, ...newEvents];
      setEvents(updated);
      localStorage.setItem('eventz_workflow_event_log', JSON.stringify(updated));
      setUpdateForm({ entryId: '', newStartMonth: '', newEndMonth: '' });
    } catch (e) {
    ...
    <div className="workflow-main-container">
      ...
      <section className="workflow-content">
        <div className="event-stream-section" style={{ position: 'relative' }}>
          ...
          <div className="buttons-row" style={{ display: 'flex', alignItems: 'center', gap: '12px', flexDirection: 'row', marginBottom: '16px' }}>
            <button
            ...
            >
              {periodResult ? 'Fermer Périodes à prendre en compte' : 'Afficher Périodes à prendre en compte'}
            </button>
          </div>
          {queryResult && (
          ...
          {periodResult && (
            <div className="projection-popup">
              <button className="projection-close-btn" onClick={handlePeriodQuery}>
                Fermer
              </button>
              <strong>Périodes à prendre en compte:</strong>
              <pre style={{ fontSize: 13, margin: 0, background: '#222', color: '#fff', padding: 16, borderRadius: 8 }}>
                {typeof periodResult === 'string' ? periodResult : JSON.stringify(periodResult, null, 2)}
              </pre>
            </div>
          )}
          {showChangeHistory && (
            <div className="projection-popup">
              <button className="projection-close-btn" onClick={() => setShowChangeHistory(false)}>
                Fermer
              </button>
              <strong>Change History (Projection):</strong>
              <pre style={{ fontSize: 13, margin: 0, background: '#222', color: '#fff', padding: 16, borderRadius: 8 }}>
                {JSON.stringify(getMutationProjection(eventLog), null, 2)}
              </pre>
            </div>
          )}
          <div className="form-row">
            <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} disabled={!canEditEntries}>
              <option value="income">Income</option>
              <option value="expense">Expense</option>
            </select>
            <select value={form.code} onChange={e => setForm({ ...form, code: e.target.value })} disabled={!canEditEntries}>
              <option value="">Code</option>
              {(form.type === 'income' ? incomeOptions : expenseOptions).map(o => (
                <option key={o.code} value={o.code}>{o.code} - {o.label}</option>
              ))}
            </select>
            <input type="number" placeholder="Amount" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} disabled={!canEditEntries} />
            <input type="month" placeholder="Start Month" value={form.startMonth} onChange={e => setForm({ ...form, startMonth: e.target.value })} disabled={!canEditEntries} />
            <input type="month" placeholder="End Month" value={form.endMonth} onChange={e => setForm({ ...form, endMonth: e.target.value })} disabled={!canEditEntries} />
            <button onClick={handleAdd} disabled={!canEditEntries}>Add</button>
          </div>
              ...
              {filteredEntries.map((row, idx) => {
                ...
                return (
                  <tr key={row.entryId}>
                    ...
                    <td>
                      <button className="btn-delete" onClick={() => handleDelete(row.entryId)} disabled={!canEditEntries}>Delete</button>
                      <button className="btn-update" onClick={() => handleUpdate(row.entryId)} disabled={!canEditEntries}>Update</button>
                      {updateForm.entryId === row.entryId && (
                        <div className="update-modal" style={{
                          ...
                          <form className="finance-form" onSubmit={handleUpdateSubmit} style={{
                            background: '#fff',
                            borderRadius: 12,
                            ...
                            <h3 style={{ marginTop: 0 }}>Modifier l'entrée</h3>
                            <div className="finance-form-row" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                              <label>Entry ID
                                <input
                              ...
                              <label>New Start Month
                                <input
                                  className="finance-input"
                                  type="month"
                                  value={updateForm.newStartMonth}
                                  onChange={e => setUpdateForm({ ...updateForm, newStartMonth: e.target.value })}
                                  required
                                />
                              </label>
                              <label>New End Month
                                <input
                                  className="finance-input"
                                  type="month"
                                  value={updateForm.newEndMonth}
                                  onChange={e => setUpdateForm({ ...updateForm, newEndMonth: e.target.value })}
                                  required
                                />
                              </label>
                            </div>
                            <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                              <button className="finance-btn" type="submit">Confirmer</button>
                              <button type="button" className="finance-btn" onClick={() => setUpdateForm({ entryId: '', newStartMonth: '', newEndMonth: '' })}>Annuler</button>
                            </div>
                            {updateError && <span style={{ color: 'red', marginLeft: 8 }}>{updateError}</span>}
                          </form>
                        </div>
                      )}
                    </td>
      ...
      <section className="workflow-event-stream">
        <EventStream
          events={eventLog}
          filter={e => e.event === 'EntryAdded' || e.event === 'EntryDeleted' || e.event === 'EntryUpdated'}
          maxHeight={400}
          showTitle={true}
        />
      </section>
