...
export function getPaymentPlanState(eventLog) {
  ...
  // Extract payments from the latest payment plan event (preserve transactionId)
  let payments = [];
  if (latestPlan && Array.isArray(latestPlan.payments)) {
    payments = latestPlan.payments.map(p => ({
      month: p.month,
      amount: p.amount,
